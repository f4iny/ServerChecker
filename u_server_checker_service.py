...
class server_checker_service:
        ...
        def is_alive(self):
                check1 = subprocess_run(["systemctl", "is-system-running"], capture_output = True, text = True, encoding = "utf-8").stdout
...
if not re.search(r'(DEBIAN|UBUNTU)', platform_freedesktop_os_release()["ID"], flags = re.IGNORECASE):
        print("Данный скрипт работает только на Debian/Ubuntu")
